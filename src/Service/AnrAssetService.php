    ...
    public function importFromArray($monarc_version, $data, $anr, &$objectsCache = [])
    {
        if(isset($data['type']) && $data['type'] == 'asset')
        {
          ...
          if (version_compare ($monarc_version, "2.8.2")>=0 ) { //TO DO:set the right value with the uuid version
            ...
            $asset = current($this->get('table')->getEntityByFields(['anr' => $anr->getId(), 'uuid' => $data['asset']['uuid']]));
            if (!empty($asset)) {
            ...
            if (!empty($data['amvs']) && !empty($idAsset)) {
              ...
              ******/
                ...
                foreach ($data['threats'] as $keyThreat => $valueThreat) {
                  $threat = $this->get('threatTable')->getEntityByFields(['anr' => $anr->getId(), 'uuid' => $keyThreat]);
              ...
              ******/
              $currentAmvs = $this->get('amvTable')->fetchAllFiltered(['uuid'],1,0,null,null,['anr'=>$anr->getId()],null,null);
                $currentAmvs = array_map(function ($elt) {
                    return (string)$elt['uuid'];
                }, $currentAmvs);
                foreach ($data['amvs'] as $keyAmv => $valueAmv) {
                  if (!in_array((string)$valueAmv['uuid'],$currentAmvs)) {//the $amv doesn't exist
                    ...
                    if (isset($valueAmv['measures']) && is_array($valueAmv['measures'])) {
                        foreach ($valueAmv['measures'] as $keyMeasure) {
                            ...
                            if (empty($measure)) {
                                $referential = current($this->get('referentialTable')->getEntityByFields(['anr' => $anr->getId(), 'uuid' => $data['measures'][$keyMeasure]['referential']['uuid']]));
                                ...
                                $category =  current($this->get('soaCategoryTable')->getEntityByFields([
                                  'anr' => $anr->getId(),
                ...
                $amvsToDelete = [];
                $oldAmvs = $this->get('amvTable')->getEntityByFields([
                    'anr' => $anr->getId(),
                    'asset' => [
                        'anr' => $anr->getId(),
                        'uuid' => $idAsset
            ...
            return $idAsset;
              ...
              // Lookup if we already have the same asset, in which case we'll update it from the data. Otherwise,
              // we'll create a new one.
              $asset = current($this->get('table')->getEntityByFields(['anr' => $anr->getId(), 'code' => $data['asset']['code']]));
              if (!empty($asset)) {
                  $idAsset = $asset->getUuid();
              } else {
                  $c = $this->get('table')->getEntityClass();
                  ...
                  foreach ($data['amvs'] as $amvArray) {
                      ...
                      if (isset($data['threats'][$amvArray['threat']])) { // Threats
                          if (is_array($data['threats'][$amvArray['threat']])) {
                              ...
                              $threat = $this->get('threatTable')->getEntityByFields(['anr' => $anr->getId(), 'code' => $data['threats'][$amvArray['threat']]['code']]);
                              if ($threat) {
                                  $threat = current($threat);
                      ...
                      if (isset($data['vuls'][$amvArray['vulnerability']])) { // Vulnerabilities
                          if (is_array($data['vuls'][$amvArray['vulnerability']])) {
                              $vul = $this->get('vulnerabilityTable')->getEntityByFields(['anr' => $anr->getId(), 'code' => $data['vuls'][$amvArray['vulnerability']]['code']]);
                              if ($vul) {
                                  $vul = current($vul);
                        ...
                        if(array_key_exists ( 'measure1' , $amvArray)){ //old version without uuid
                          //we need to create ISO 27002, we check if the common if it's present or not
                            $referential = false;
                            $referentialCli = current($this->get('referentialTable')->getEntityByFields(['anr' => $anr->getId(), 'uuid' =>'98ca84fb-db87-11e8-ac77-0800279aaa2b']));
                            if(!$referentialCli)
                          ...
                          for ($i = 1; $i <= 3; $i++) {
                            if (isset($data['measures'][$amvArray['measure' . $i]])) { // Measure 1 / 2 / 3
                                if (is_array($data['measures'][$amvArray['measure' . $i]])) {
                                    $measure = $this->get('measureTable')->getEntityByFields(['anr' => $anr->getId(), 'code' => $data['measures'][$amvArray['measure' . $i]]['code']]);
                                    if ($measure) {
                      ...
                      if (empty($amvTest)) { // on test que cet AMV sur cette ANR n'existe pas
                          ...
                          $MonarcObjectTable = $this->get('MonarcObjectTable');
                          $objects = $MonarcObjectTable->getEntityByFields(['anr' => $anr->getId(), 'asset' => ['anr' => $anr->getId(), 'uuid' =>$idAsset]]);
                          foreach ($objects as $object) {
                              /** @var InstanceTable $instanceTable */
                              $instanceTable = $this->get('instanceTable');
                              $instances = $instanceTable->getEntityByFields([
              ...
              */
              if (empty($localAmv)) {
                   $risks = $this->get('instanceRiskTable')->getEntityByFields(['asset'=>['anr'=>$anr->getId(),'uuid' => $idAsset], 'anr'=>$anr->getId(),
