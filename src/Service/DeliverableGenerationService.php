      ...
      for ($i=1; $i <= 4 ; $i++) {
        ...
        if(!empty($risksByTreatment)){
          ...
          foreach ($risksByTreatment as $r) {
            ...
            $tableRiskInfo->addCell(Font::centimeterSizeToTwips(3.50), $styleContentCell)->addText(_WT($r['threatLabel' . $anr->language]), $styleContentFont, $alignLeft);
            $tableRiskInfo->addCell(Font::centimeterSizeToTwips(2.00), $styleContentCell)->addText($r['threatRate'], $styleContentFont, $alignCenter);
            $tableRiskInfo->addCell(Font::centimeterSizeToTwips(4.00), $styleContentCell)->addText(_WT($r['vulnLabel' . $anr->language]), $styleContentFont, $alignLeft);
        ...
        if(!empty($risksOpByTreatment)){
          ...
          foreach ($risksOpByTreatment as $r) {
              ...
              $tableRiskOp->addCell(Font::centimeterSizeToTwips(10.00), $styleContentCell)->addText(_WT($r['label' . $anr->language]), $styleContentFont, $alignLeft);
              if ($anr->showRolfBrut == 1) {
        ...
        foreach ($recosRisks as $recoRisk) {
            if ($recoRisk->instanceRisk) {
                ...
                if ($continue) {
                    ...
                    $table->addCell(Font::centimeterSizeToTwips(6.00), $styleContentCell)->addText(_WT($recoRisk->threat->{'label' . $anr->language}), $styleContentFont, $alignLeft);
                    $table->addCell(Font::centimeterSizeToTwips(6.00), $styleContentCell)->addText(_WT($recoRisk->vulnerability->{'label' . $anr->language}), $styleContentFont, $alignLeft);
            ...
            if ($recoRisk->instanceRiskOp) {
                ...
                if (empty($recoRisk->objectGlobal->uuid)) {
                  ...
                  $path = $recoRisk->instance->{'name' . $anr->language} . ' (' . $this->anrTranslate('Global') . ')';
                    ...
                    $table->addRow(400);
                    $table->addCell(Font::centimeterSizeToTwips(3.00), $styleContentCell)->addText(_WT($path), $styleContentFont, $alignLeft);
						        $table->addCell(Font::centimeterSizeToTwips(12.20), $styleContentCell)->addText(_WT($recoRisk->instanceRiskOp->{'riskCacheLabel' . $anr->language}), $styleContentFont, $alignLeft);
    ...
    protected function generateTableStatementOfApplicability($anr, $referential)
    {
       ...
       foreach ($controlSoaList as $controlSoa) {
           ...
           if ($controlSoa['measure']->category->id != $previousCatId) {
             $table->addRow(400);
             $table->addCell(Font::centimeterSizeToTwips(10.00), $styleContentCellCat)->addText(_WT($controlSoa['measure']->category->get('label' . $anr->language)), $styleContentFontBold, $alignLeft);
           }
           ...
           $table->addCell(Font::centimeterSizeToTwips(5.00), $styleContentCell)->addText(_WT($controlSoa['measure']->get('label' . $anr->language)), $styleContentFont, $alignLeft);
        ...
        if (!empty($controlSoa['measure']->amvs) || !empty($controlSoa['measure']->rolfRisks)) {
          ...
          if (!empty($controlSoa['measure']->amvs)) {
            ...
            foreach ($controlSoa['measure']->amvs as $r) {
              ...
              $tableRiskInfo->addCell(Font::centimeterSizeToTwips(0.70), $styleContentCell)->addText($r['i_impact'], $styleContentFont, $alignCenter);
              $tableRiskInfo->addCell(Font::centimeterSizeToTwips(0.70), $styleContentCell)->addText($r['d_impact'], $styleContentFont, $alignCenter);
              $tableRiskInfo->addCell(Font::centimeterSizeToTwips(2.50), $styleContentCell)->addText(_WT($r['threatLabel' . $anr->language]), $styleContentFont, $alignLeft);
              $tableRiskInfo->addCell(Font::centimeterSizeToTwips(2.00), $styleContentCell)->addText($r['threatRate'], $styleContentFont, $alignCenter);
              $tableRiskInfo->addCell(Font::centimeterSizeToTwips(3.00), $styleContentCell)->addText(_WT($r['vulnLabel' . $anr->language]), $styleContentFont, $alignLeft);
              $tableRiskInfo->addCell(Font::centimeterSizeToTwips(4.00), $styleContentCell)->addText(_WT($r['comment']), $styleContentFont, $alignLeft);
    ...
    */
    protected function generateTableRecordProcessors($anr, $recordId)
    {
        ...
        foreach ($processors as $p) {
            ...
            $table->addCell(Font::centimeterSizeToTwips(14.00), $styleContentCell)->addText(_WT($p->get('activities')[$recordId]), $styleContentFont, $alignLeft);
            $table->addRow(400);
            $table->addCell(Font::centimeterSizeToTwips(4.00), $styleHeaderCell)->addText($this->anrTranslate('Security measures'), $styleHeaderFont, $alignLeft);
            ...
            $tableActor->addCell(Font::centimeterSizeToTwips(6.00), $styleHeaderCell)->addText($this->anrTranslate('Actor'), $styleHeaderFont, $alignCenter);
            $tableActor->addCell(Font::centimeterSizeToTwips(6.00), $styleHeaderCell)->addText($this->anrTranslate('Name'), $styleHeaderFont, $alignCenter);
            $tableActor->addCell(Font::centimeterSizeToTwips(6.00), $styleHeaderCell)->addText($this->anrTranslate('Contact'), $styleHeaderFont, $alignCenter);

            $tableActor->addRow(400);
            $tableActor->addCell(Font::centimeterSizeToTwips(6.00), $styleHeaderCell)->addText($this->anrTranslate('Representative'), $styleHeaderFont, $alignLeft);
            $tableActor->addCell(Font::centimeterSizeToTwips(6.00), $styleContentCell)->addText(_WT($p->get('representative')? $p->get('representative')->get('label') : ''), $styleContentFont, $alignLeft);
            $tableActor->addCell(Font::centimeterSizeToTwips(6.00), $styleContentCell)->addText(_WT($p->get('representative')? $p->get('representative')->get('contact') : ''), $styleContentFont, $alignLeft);

            $tableActor->addRow(400);
            $tableActor->addCell(Font::centimeterSizeToTwips(6.00), $styleHeaderCell)->addText($this->anrTranslate('Data protection officer'), $styleHeaderFont, $alignLeft);
            $tableActor->addCell(Font::centimeterSizeToTwips(6.00), $styleContentCell)->addText(($p->get('dpo')? $p->get('dpo')->get('label') : ''), $styleContentFont, $alignLeft);
            $tableActor->addCell(Font::centimeterSizeToTwips(6.00), $styleContentCell)->addText(($p->get('dpo')? $p->get('dpo')->get('contact') : ''), $styleContentFont, $alignLeft);
            $section->addTextBreak(1);
        }
        return $this->getWordXmlFromWordObject($tableWord);
    }
                ...
                foreach ($instanceConsequences as $keyConsequence => $instanceConsequence) {
                    if ($instanceConsequence[$impact . '_risk'] >= 0) {
                        if (!$headerImpact && !$headerConsequence) {
                            $table->addRow(400);
                            $table->addCell(Font::centimeterSizeToTwips(16), $cellColSpan)->addText(_WT($i['name' . $anr->language]), $styleHeaderFont, $alignLeft);
                        }
                        $table->addRow(400);
                        ...
                        $table->addCell(Font::centimeterSizeToTwips(1.00), $styleContentCell)->addText(_WT($instanceConsequence['scaleImpactTypeDescription' . $anr->language]), $styleContentFontBold, $alignCenter);
    ...
    protected function generateThreatsTable($anr, $fullGen = false)
    {
        ...
        foreach ($threats as $threat) {
            if (($threat['trend'] != 1) || $fullGen) { // All but normal
                $table->addRow(400);
                $table->addCell(Font::centimeterSizeToTwips(5.85), $styleContentCell)->addText(_WT($threat['label' . $anr->language]), $styleContentFont, $styleContentParagraphLeft);
