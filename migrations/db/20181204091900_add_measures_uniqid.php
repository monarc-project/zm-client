...
class AddMeasuresUniqid extends AbstractMigration
{
    ...
    public function change()
    {
      ...
      $table->dropForeignKey('measure_id')
            ->addColumn('measure_uniqid', 'uuid',array('after' => 'id'))
            ->update();
      $this->execute('UPDATE recommandations_measures MA,measures M SET MA.measure_uniqid = M.uniqid where M.id=MA.measure_id');
      $table->removeColumn('measure_id')
            ->renameColumn('measure_uniqid','measure_id')
            ->update();
      ...
      $table->dropForeignKey('measure_id')
            ...
            ->update();
      $this->execute('UPDATE soa MA,measures M SET MA.measure_uniqid = M.uniqid where M.id=MA.measure_id');
      $table->removeColumn('measure_id')
            ->renameColumn('measure_uniqid','measure_id')
            ->update();
